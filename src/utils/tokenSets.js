import bessemerImg from '../assets/Brass-Birmingham/cropped/bessemer.png'
import owenImg from '../assets/Brass-Birmingham/cropped/owen.png'
import arkwrightImg from '../assets/Brass-Birmingham/cropped/arkwright.png'
import wattImg from '../assets/Brass-Birmingham/cropped/watt.png'
import brunelImg from '../assets/Brass-Birmingham/cropped/brunel.png'
import stephensonImg from '../assets/Brass-Birmingham/cropped/stephenson.png'
import tinsleyImg from '../assets/Brass-Birmingham/cropped/tinsley.png'
import coadeImg from '../assets/Brass-Birmingham/cropped/coade.png'
...
// actual in-game piece can be looked up directly instead of guessed, and
// either a shape/color (an SVG path + its own viewBox, since each piece's
...
// `slug` alone is what resolves the localized display name, via
// `t(\`meeples.${gameKey}.${slug}\`)` (see src/i18n/locales) since it's
// user-facing text.
//
// TOKEN_SETS is keyed by each game's BoardGameGeek id (`bggId`, as returned
// alongside `name` by /api/locations/:id/games) rather than by the game's
// display name or this app's own per-deployment catalog `id`. A display
// name can vary in punctuation/capitalization (e.g. "Brass: Birmingham" vs
// "Brass Birmingham") and a catalog `id` is only assigned when a game is
...
export const TOKEN_SETS = {
  ...
  //
  // Unlike Everdell's flat single-path silhouettes, these are cropped
  // portrait photos (see src/assets/Brass-Birmingham/) — an `image` token
 ...
 *
 * `bggId` should come from the game's catalog entry (see
 * /api/locations/:id/games), not parsed out of its display name.
 */
export function getMeepleOptions(bggId) {
  const set = TOKEN_SETS[bggId]
  if (!set) return []
  return set.tokens.map((token) => ({ ...token, gameKey: set.gameKey }))
}
